...
1. Open `ios/DraftFirst.xcodeproj` in Xcode 26.6 or newer.
2. Select the **DraftFirst** scheme and an iOS 26 simulator or device.
3. Build and run.
...
All screenplay intelligence is native Swift in `ios/DraftFirstEngine` (linked as a local package): element choreography, text normalization, Fountain parsing and serialization, deterministic pagination, and the prediction engine. Behaviour is pinned to the TypeScript reference engine by a conformance corpus — regenerate the fixtures from the repository root after changing the TypeScript engine, then run the package tests:

```sh
npm run engine:conformance
cd ios/DraftFirstEngine && swift test
```
