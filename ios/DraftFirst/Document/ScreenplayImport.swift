...
enum ScreenplayImport {
    ...
    static let readableTypes: [UTType] = [
        .draftFirstScreenplay, .finalDraftScreenplay, .pdf, .plainText, .image
    ]
    ...
    static func document(at url: URL) async throws -> DraftFirstDocument {
        DraftFirstDocument(source: try await source(at: url))
    }
    ...
    static func source(at url: URL) async throws -> String {
        ...
        return try DraftFirstDocument.decode(try Data(contentsOf: url), as: type)
    }
