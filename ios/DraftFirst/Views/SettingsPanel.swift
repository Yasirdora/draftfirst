...
/// Screenplay metadata: the title-page fields, presented from the document
/// menu's "Title Page…". There is deliberately no file-name field — the
/// title is the screenplay's identity, and file management stays in
/// Documents.
struct TitlePageSettings: View {
    let editor: EditorState
    ...
    init(editor: EditorState) {
        self.editor = editor
        _title = State(initialValue: editor.titlePageValue(for: "Title") ?? "")
        _writer = State(initialValue: editor.titlePageValue(for: "Author") ?? "")
        _credit = State(initialValue: editor.titlePageValue(for: "Credit") ?? "written by")
    }
            ...
            Form {
                Section {
                    LabeledContent("Title") {
                        TextField("Untitled Screenplay", text: $title)
                    ...
                    LabeledContent("Credit") {
                        TextField("written by", text: $credit)
                            .textInputAutocapitalization(.sentences)
                            .multilineTextAlignment(.trailing)
                            .submitLabel(.done)
                    }
                } footer: {
                    Text("Appears on the screenplay's title page and on the PDF you send out.")
                }
            }
    ...
    private func save() {
        editor.updateTitlePage(title: title, writer: writer, credit: credit)
    }
}
