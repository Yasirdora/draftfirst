...
1. Open `apple/eDraft.xcodeproj` in Xcode 26.6 or newer.
...
All screenplay intelligence is native Swift in `apple/eDraftEngine` (linked as a local package): element choreography, text normalization, Fountain parsing and serialization, deterministic pagination, and the prediction engine. Behaviour is pinned to the TypeScript reference engine by a conformance corpus — regenerate the fixtures from the repository root after changing the TypeScript engine, then run the package tests:

```sh
npm run engine:conformance
cd apple/eDraftEngine && swift test
```
