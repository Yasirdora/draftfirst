...
final class LaunchModelTests: XCTestCase {
    ...
    func testTheListIsCappedSoTheWindowStaysAWindow() {
        let many = (1...(LaunchModel.limit + 12)).map { url("Script \($0).draft") }
    ...
    func testFavouritesAreMarkedAndListedFirst() {
        let a = url("A.draft"), b = url("B.draft")
        let rows = LaunchModel.rows(from: [a, b], favorites: [b], exists: { _ in true }, modified: { _ in nil })
        XCTAssertEqual(rows.map(\.isFavorite), [false, true])
        XCTAssertEqual(LaunchModel.filtered(rows, query: "").map(\.name), ["B", "A"])
        XCTAssertEqual(LaunchModel.filtered(rows, query: "a").map(\.name), ["A"])
    }
}
