/**
 * The numbered scene heading grammar (routing pack, step 1 — corpus-witnessed).
 *
 ...
 * HOLE." (corpus-6.txt, 40 witnesses). An omitted scene keeps its number and
 * prints the OMITTED card, numbered or bare: "113 OMITTED." (corpus-6),
 * "OMITTED" (episode-101, whiplash).
 *
...
const FLANKING_NUMBER = /\s+(\d+)\*?$/;
const OMITTED_CARD = /^(\d+\s+)?OMITTED\.?$/;
 ...
 * card parses with no number; its trailing period is not meaning.
 */
