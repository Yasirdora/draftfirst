...
```bash
npm test                                              # 627 TypeScript (598 engine package + 29 web app)
npm run check:paste-corpus                            # 15 scripts hold (skips without the corpus)
swift test --package-path apple/eDraftEngine          # 145 engine
swift test --package-path apple/EDraftCore            # 270 core        (macOS)
...
*Recounted 2026-09-13 (third pass, at 23d0b78): TypeScript 627 (598 engine +
29 web), core 270, document 29 and Mac surface 238 were run against the tip in
a scratch worktree, so the other sessions' uncommitted files (zoom, scene
filter) are not in these numbers; the engine's 145 and the paste-corpus gate's
15 scripts were run in the working tree (nothing uncommitted touches them).
...
| [RFC-SECONDARY-SLUG.md](RFC-SECONDARY-SLUG.md) | Secondary slugs typed `.scene` (numbering honours `isSecondary`), the pasted title-page block (credit-anchored), `THE END` as a closing card, the typed OMITTED card — the third paste-routing pass |
